...
class Kiwoom(QAxWidget):
    ...
    def trdata_slot(self,sScrNo,sRQName,sTrCode,sRecordName,sPrevNext):
        ...
        if sRQName == "주식일봉차트조회":
            ...
            cnt = self.dynamicCall("GetRepeatCnt(QString, QString)", sTrCode,sRQName)  # 멀티데이터의 정보를 가져오겠다는뜻(GetRepeatCnt
            print("데이터일수 %s " % cnt)


            #한번 조회하면 600일치까지 일봉 데이터를 받을 수 있다.
            for i in range(cnt):
                data =  []
                current_price = self.dynamicCall("GetCommData(QString,QString,int,QString)",sTrCode,sRQName,i,"현재가")
            ...
            if sPrevNext == "2":
                ...
                if self.calcul_data == None or len(self.calcul_data) < 120:
                    ...
                    for value in self.calcul_data[:120]:
                        total_price += int(value[1]) #value[1] 현재가, 일봉에서 현재가는 종가

                    moving_average_price = total_price / 120 #120일 이평선
                    ...
                    if bottom_stock_price == True:
                        ...
                        if price_top_moving == True:
                            if moving_average_price > moving_average_price_prev and check_price > prev_price:
                                ...
                                print("포착괸 부분의 일봉 저가가 오늘자 일본의 고가보다 낮은지 확인됨")
                                pass_success = True
                if  pass_success == True:
                    print("조건부 통과됨")
                    code_nm = self.dynamicCall("GetMasterCodeName(QString)", code)
                    f = open("files/condition_stock.txt","a",encodign="utf8")
    ...
    def get_code_list_my_market(self,market_code):
        ...
        :return:
        """
        code_list = self.dynamicCall("GetCodeListByMarket(QString)",market_code) #코스닥종목번호 다 받아와라
        ...
        if os.path.exists("files/condition_stock.txt"): #파일이 있냐?
            ...
            for line in lines:
                if line != "":
                    ...
                    stock_code = ls[0]
                    stock_name = ls[1]
                    stock_price = int(ls[2].split("\n")[0]) #split로 나누면 뒤에 공백이 나오는데 앞에것만 가져오려고
                    stock_price = abs(stock_price) #마이너스를 절대값 즉 양수로 만들어주기 위해서
    ...
    def screen_number_setting(self):
        ...
        for code in screen_overwrite:
            ...
            meme_screen = int(self.screen_meme_stock) #매매용 스크린번호
    ...
    def realdata_slot(self, sCode, sRealType, SrealData):
        ...
        if sRealType == "장시작시간":
            ...
            l = self.dynamicCall("GetCommRealData(QString,int)", sCode, self.realType.REALTYPE[sRealType]['거래비용'])
            l = abs(int(l))
            ...
            if sCode in self.account_stock_dict.keys() and sCode not in self.jango_dict.keys():
                ...
                if asd['매매가능수량'] > 0 and (meme_rate > 5 or meme_rate < -5):
                    ...
                    if order_success == 0:
                        print("매도주문 전달 성공")
                        del self.account_stock_dict[sCode] #매도가 주문되면 딕셔너리에서도 지워라
                    else:
                        print("매도주문 전달 실패")
            ...
            #오늘 산 잔고에 있을 경우
                ...
                if jd['주문가능수량'] > 0 and (meme_rate > 5 or meme_rate < -5):
                    ...
                    if order_success == 0:
                        ...
                        self.logging.logger.debug("매도주문 전달 실패패")



            #등략율이 2.0% 이상이고 오늘 산 잔고에 없을 경우우
                ...
                order_success = self.dynamicCall("SendOrder(QString,QString,QString,int,QString,int,int,QString,QString)",
                    ["신규매수", self.portfolo_stock_dict[sCode]['주문용스크린번호'], self.account_num, 1,sCode, quantity, e, self.realType.SENDTYPE['거래구분']['지정가'], ""])
            ...
            for order_num in not_meme_list:
                ...
                if order_gubun == "매수" and not_quantity > 0 and e > meme_price:
                    ...
                    order_success = self.dynamicCall("SendOrder(QString,QString,QString,int,QString,int,int,QString,QString)",
                        ["매수취소", self.portfolo_stock_dict[sCode]['주문용스크린번호'], self.account_num, 3, code, 0, 0,
